...
export default function CardsAdminProf({ id }) {
  ...
  const allProfOrd = allPorfessionals.sort((a, b) =>
    a.name > b.name ? 1 : -1
  );
  ...
  const handlerDelete = async (id) => {
    ...
    if (confirmDelete.isConfirmed) {
      const turnIdsToDelete = turns
        .filter((turn) => turn.professionalID === id)
        .map((turn) => turn.id);
      const deleteTurnPromises = turnIdsToDelete.map((turnId) =>
  ...
  const disabledProfessionals = allProfOrd.filter(
    (professional) => !professional.disponibility
  );
  const enabledProfessionals = allProfOrd.filter(
    (professional) => professional.disponibility
  );
